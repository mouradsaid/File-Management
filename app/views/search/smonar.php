        ...
        <?php if(!empty($data['data']) ){foreach ($data['data'] as $file) : ?>
          <tr>
            ...
            <td>
              ...
              <a href="<?php echo URLROOT.'files/downlodar/'.$file->nameDoc ; ?>" class="deletem" ><span class="material-icons">download</span></a>&nbsp;
              <a href="<?php echo URLROOT.'files/unarchive/'.$file->id; ?>" class="deletem1" ><span class="material-icons">unarchive</span></a>&nbsp;
              <?php 
...
$(".pageSearch").on('click',function(event){
            ...
            var input=$("#idinput").val();
             $.ajax({
                         url:"<?php echo URLROOT;  ?>files/searcharchive",
                         method:"post",
...
<div id="editEmployeeModal<?php echo  $file->id ; ?>" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content">
      <form action="<?php echo URLROOT;  ?>files/editarchive/<?php echo  $file->id ; ?>" method="post">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <?php for($i=0;$i<count($data['type']);$i++){ echo "<option value=".$data['type'][$i].">".$data['type'][$i]."</option>"; }  ?>
            </select>
          </div>
          <div class="form-group">
            <label>Localisation</label>
            <select name="dlocalisation" id="" class="form-control" >
            <?php for($i=0;$i<count($data['localisationTags']);$i++){ echo "<option value=".$data['localisationTags'][$i].">".$data['localisationTags'][$i]."</option>"; }  ?>
            </select>
          </div>
        </div>
...
<div id="deleteEmployeeModal<?php echo  $file->id ; ?>" class="modal fade">
  <div class="modal-dialog">
    <div class="modal-content">
      <form  action="<?php echo URLROOT;  ?>files/deleteFilear/<?php echo  $file->id ; ?>" method="post" >
        <div class="modal-header">						
